%% run_time_oscillator_analysis_lighttap.m
...
%   - calls temporal_pattern_test_one_recording_v1 for each recording
...
outDir = fullfile(baseFolder, 'time_oscillator_analysis_results_v2');
...
for iRec = 1:numel(validFolders)
    ...
    try
        ...
        OUTi.recordingLabel = fishLabel{iRec};
        OUTi.folder = validFolders{iRec};
        OUTi.resultsFile = resultsFiles{iRec};
        OUTi.metaFile = metaFiles{iRec};

        if cfg.saveFigures && cfg.doPlots
            save_one_figure_if_present(OUTi.fig, 'summary',        figDir, [fishLabel{iRec} '_summary.png']);
            save_one_figure_if_present(OUTi.fig, 'phase',          figDir, [fishLabel{iRec} '_phase.png']);
            save_one_figure_if_present(OUTi.fig, 'sliding',        figDir, [fishLabel{iRec} '_sliding.png']);
            save_one_figure_if_present(OUTi.fig, 'modeMembership', figDir, [fishLabel{iRec} '_modeMembership.png']);
            save_one_figure_if_present(OUTi.fig, 'modeDynamics',   figDir, [fishLabel{iRec} '_modeDynamics.png']);
            save_one_figure_if_present(OUTi.fig, 'neuronMap',      figDir, [fishLabel{iRec} '_neuronMap.png']);
        end

        AllRes = [AllRes; OUTi]; %#ok<AGROW>
        ...
        row.meanSlidingPairs    = mean(OUTi.sliding.nPairsLDS, 'omitnan');
        row.maxSlidingPairs     = max(OUTi.sliding.nPairsLDS, [], 'omitnan');
        row.interpretation      = string(OUTi.summary.interpretation);

        row.pc1_domFreqHz       = safe_idx(OUTi.summary.pcDomFreqHz, 1);
        row.pc2_domFreqHz       = safe_idx(OUTi.summary.pcDomFreqHz, 2);
        row.pc3_domFreqHz       = safe_idx(OUTi.summary.pcDomFreqHz, 3);
        row.pc4_domFreqHz       = safe_idx(OUTi.summary.pcDomFreqHz, 4);
        row.pc5_domFreqHz       = safe_idx(OUTi.summary.pcDomFreqHz, 5);
        row.pc6_domFreqHz       = safe_idx(OUTi.summary.pcDomFreqHz, 6);

        row.expVar_PC1          = safe_idx(OUTi.tpca.explained, 1);
        row.expVar_PC2          = safe_idx(OUTi.tpca.explained, 2);
        row.expVar_PC3          = safe_idx(OUTi.tpca.explained, 3);
        row.expVar_PC4          = safe_idx(OUTi.tpca.explained, 4);
        row.expVar_PC5          = safe_idx(OUTi.tpca.explained, 5);
        row.expVar_PC6          = safe_idx(OUTi.tpca.explained, 6);

        row.nAcfFilteredOut     = sum(~OUTi.filter.keepFinal);
        row.nAcfKept            = sum(OUTi.filter.keepFinal);

        row.mode1_n             = sum(OUTi.mode.modeID == 1);
        row.mode2_n             = sum(OUTi.mode.modeID == 2);
        row.mode3_n             = sum(OUTi.mode.modeID == 3);
        row.mode4_n             = sum(OUTi.mode.modeID == 4);
        row.mode5_n             = sum(OUTi.mode.modeID == 5);
        row.mode6_n             = sum(OUTi.mode.modeID == 6);

        row.nOccupiedModes      = nnz(OUTi.summary.modeCount > 0);
