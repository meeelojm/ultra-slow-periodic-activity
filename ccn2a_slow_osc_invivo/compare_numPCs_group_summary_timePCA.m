function OUT = compare_numPCs_group_summary_timePCA(baseFolder, varargin)
%COMPARE_NUMPCS_GROUP_SUMMARY
% Group-level sweep over latent dimensionality for temporal-pattern PCA analysis.
%
% Required:
...
%   2) runs a transpose-PCA / temporal-pattern analysis for each recording,
%      sweeping numPCs in [3 4 5 6 8] by default
%   3) computes compact group summaries to help choose how many temporal
%      patterns are worth retaining
%
% Important interpretation:
%   This is NOT the original population-mode PCA formulation.
%   Here, PCA is performed with:
%       - neurons as observations
%       - timepoints as variables
%   so the retained components are interpreted as dominant temporal motifs
%   or temporal patterns, and each neuron is assigned according to how
%   strongly it loads onto those temporal patterns.
%
% Main outputs per numPCs:
%   - explained variance retained by temporal PCs
%   - occupied temporal patterns
%   - mean PLV among temporal patterns
%   - nPhaseComponents in temporal-pattern phase space
%   - oscillator pairs (whole-recording and sliding-window) detected from
%     temporal-pattern trajectories
%   - geometric separation of neurons in temporal-PC score space
%   - stability of neuron assignments vs previous numPCs
%
% Use this function when you want to ask:
%   - how many dominant temporal motifs describe the recording?
%   - are those temporal motifs oscillatory, weakly coupled, or fragmented?
%   - do neurons cluster stably by their loading onto temporal motifs as K changes?
%
% Example:
...
for iK = 1:numel(numPCsList)
    ...
    OUTi = temporal_pattern_test_one_recording_v1( ...
        ...
        expRetained = sum(OUTi.tpca.explained, 'omitnan');
...
%   mode.modeID           : dominant temporal-pattern assignment per neuron
...
if P.EnableAcfPeakFilter
    ...
    for i = 1:N0
        ...
        pks = findpeaks(aciWin, 'MinPeakProminence', P.AcfMinProminence); %#ok<NASGU>
...
% score  -> neuron weights    [neurons x K]
...
% Scale temporal patterns by sqrt(latent) for more interpretable amplitude
...
%% ======================================================================
function [latentBand, latentPhase, latentAmp, PLV, meanPLV] = compute_phase_metrics(scoreWin, fps, bandHz, phaseAmpQuantileMin)
...
latentBand = nan(size(scoreWin));
for i = 1:size(scoreWin,2)
    xi = fillmissing(scoreWin(:,i),'linear','EndValues','nearest');
...
%% ======================================================================
function [A, lambda, pairs, pairFreqHz] = fit_latent_lds(scoreWin, fps, bandHz, ldsReg, minEigMag, maxEigMag)
...
%% ======================================================================
function sliding = compute_sliding_latent_metrics(temporalPatterns, fps, slideWinSec, slideStepSec, ...
    bandHz, welchWinSec, welchOverlapFrac, phaseAmpQuantileMin, ...
    ldsReg, minEigMag, maxEigMag, plvThreshold) %#ok<INUSD>

T = size(temporalPatterns,1);
...
sliding = struct();
sliding.centerSec  = nan(nW,1);
sliding.meanPLV    = nan(nW,1);
sliding.pcFreqCV   = nan(nW,1);
sliding.nPairsLDS  = nan(nW,1);
sliding.pcDomFreq  = nan(nW, numPCs);

for w = 1:nW
    s = starts(w);
    e = min(T, s+winF-1);
    Xi = temporalPatterns(s:e,:);

    [pcDomFreq, ~, ~, ~] = compute_pc_psd(Xi, fps, welchWinSec, welchOverlapFrac, bandHz);
