...
LL.confCutOff = 99;              % confidence cutoff
...
LL.plotFlag = 0;
makePerFishRasterQC = false;
makePerFishFullQC = true;
nExampleNeuronsQC = 5;
...
allNeuronDeltaDur = cell(nR,1);
allNeuronDeltaAUC = cell(nR,1);
allRecDeltaLoc    = nan(nR,1);
...
R.neuronPair_n_dur      = nan(nR,1);
R.neuronPair_p_dur      = nan(nR,1);
R.neuronPair_delta_dur  = nan(nR,1);

R.neuronPair_n_auc      = nan(nR,1);
R.neuronPair_p_auc      = nan(nR,1);
R.neuronPair_delta_auc  = nan(nR,1);
...
for r = 1:nR
    ...
    % ---------------------------------------------------------------------
    % 3.9 Full per-fish QC panel
    % ---------------------------------------------------------------------
    if makePerFishFullQC
        try
            figFullQC = make_LL_full_per_fish_QC( ...
                eve_fra_cel, dff_dow_fra_cel, ...
                low_idx_event, high_idx_event, ...
                fps_event, R.recordingLabel{r}, nExampleNeuronsQC);

            if saveOutputs
                qcName = ['QC_full_' sanitize_filename(R.recordingLabel{r}) '.png'];
                saveas(figFullQC, fullfile(outFolder, qcName));
            end
            close(figFullQC);
        catch ME
            warning('Could not create full QC panel for %s\n%s', recordings(r).folder, ME.message);
        end
    end

    % ---------------------------------------------------------------------
    % 3.X Within-recording paired neuron tests
    % ---------------------------------------------------------------------

    perNeuronLow  = compute_per_neuron_event_features(eve_fra_cel(low_idx_event,:),  dff_dow_fra_cel(low_idx_event,:),  fps_event);
    perNeuronHigh = compute_per_neuron_event_features(eve_fra_cel(high_idx_event,:), dff_dow_fra_cel(high_idx_event,:), fps_event);
    outDur = run_paired_signrank(perNeuronLow.meanDur, perNeuronHigh.meanDur);
    outAUC = run_paired_signrank(perNeuronLow.meanAUC, perNeuronHigh.meanAUC);

    R.neuronPair_n_dur(r)     = outDur.n;
    R.neuronPair_p_dur(r)     = outDur.p;
    R.neuronPair_delta_dur(r) = outDur.deltaMedian;

    R.neuronPair_n_auc(r)     = outAUC.n;
    R.neuronPair_p_auc(r)     = outAUC.p;
    R.neuronPair_delta_auc(r) = outAUC.deltaMedian;

    % Recording-level locomotion delta
    allRecDeltaLoc(r) = R.high_boutRate_perMin(r) - R.low_boutRate_perMin(r);

    % Neuron-level deltas within this recording
    allNeuronDeltaDur{r} = perNeuronHigh.meanDur - perNeuronLow.meanDur;
    allNeuronDeltaAUC{r} = perNeuronHigh.meanAUC - perNeuronLow.meanAUC;

    fprintf('  Neuron-paired duration: n=%d, p=%.4g, median Δ=%.4g\n', ...
        outDur.n, outDur.p, outDur.deltaMedian);
    fprintf('  Neuron-paired AUC:      n=%d, p=%.4g, median Δ=%.4g\n', ...
        outAUC.n, outAUC.p, outAUC.deltaMedian);
...
T = table( ...
    ...
    R.neuronPair_n_dur, R.neuronPair_p_dur, R.neuronPair_delta_dur, ...
    R.neuronPair_n_auc, R.neuronPair_p_auc, R.neuronPair_delta_auc, ...
    'VariableNames', { ...
    ...
    'low_eventAUC_mean','high_eventAUC_mean',...
     'neuronPair_n_dur','neuronPair_p_dur','neuronPair_delta_dur', ...
    'neuronPair_n_auc','neuronPair_p_auc','neuronPair_delta_auc'});
...
%% =========================
% 5B) POOLED ACROSS ALL NEURONS / EVENTS
% =========================
% Descriptive pooled analysis across all recordings.
% IMPORTANT: this is NOT fish-level inference.

pooled = struct();

% Pool per-neuron event rates
pooled.low_rates  = vertcat_nonempty(allLowRates);
pooled.high_rates = vertcat_nonempty(allHighRates);

% Pool event amplitudes, durations, AUCs (event-level)
pooled.low_amp  = vertcat_nonempty(allLowEventAmp);
pooled.high_amp = vertcat_nonempty(allHighEventAmp);

pooled.low_dur  = vertcat_nonempty(allLowEventDur);
pooled.high_dur = vertcat_nonempty(allHighEventDur);

pooled.low_auc  = vertcat_nonempty(allLowEventAUC);
pooled.high_auc = vertcat_nonempty(allHighEventAUC);

% Pooled summary metrics
pooled.low_eventRate_mean   = mean(pooled.low_rates,  'omitnan');
pooled.high_eventRate_mean  = mean(pooled.high_rates, 'omitnan');

pooled.low_eventRate_median  = median(pooled.low_rates,  'omitnan');
pooled.high_eventRate_median = median(pooled.high_rates, 'omitnan');

pooled.low_totalEventCount  = numel(pooled.low_dur);
pooled.high_totalEventCount = numel(pooled.high_dur);

pooled.low_fracActiveNeurons  = mean(pooled.low_rates > 0,  'omitnan');
pooled.high_fracActiveNeurons = mean(pooled.high_rates > 0, 'omitnan');

pooled.low_eventAmp_mean  = mean(pooled.low_amp,  'omitnan');
pooled.high_eventAmp_mean = mean(pooled.high_amp, 'omitnan');

pooled.low_eventDur_mean  = mean(pooled.low_dur,  'omitnan');
pooled.high_eventDur_mean = mean(pooled.high_dur, 'omitnan');

pooled.low_eventAUC_mean  = mean(pooled.low_auc,  'omitnan');
pooled.high_eventAUC_mean = mean(pooled.high_auc, 'omitnan');

% Optional pooled nonparametric tests
% These are descriptive/supporting only because neurons/events are not
% independent biological replicates across fish.
pooled.stats = struct();

if numel(pooled.low_rates) >= 2 && numel(pooled.high_rates) >= 2
    pooled.stats.rate = ranksum(pooled.low_rates, pooled.high_rates);
else
    pooled.stats.rate = NaN;
end

if numel(pooled.low_amp) >= 2 && numel(pooled.high_amp) >= 2
    pooled.stats.amp = ranksum(pooled.low_amp, pooled.high_amp);
else
    pooled.stats.amp = NaN;
end

if numel(pooled.low_dur) >= 2 && numel(pooled.high_dur) >= 2
    pooled.stats.dur = ranksum(pooled.low_dur, pooled.high_dur);
else
    pooled.stats.dur = NaN;
end

if numel(pooled.low_auc) >= 2 && numel(pooled.high_auc) >= 2
    pooled.stats.auc = ranksum(pooled.low_auc, pooled.high_auc);
else
    pooled.stats.auc = NaN;
end

% Fraction active: contingency table
nLowActive   = sum(pooled.low_rates > 0,  'omitnan');
nLowInactive = sum(pooled.low_rates == 0, 'omitnan');
nHighActive   = sum(pooled.high_rates > 0,  'omitnan');
nHighInactive = sum(pooled.high_rates == 0, 'omitnan');

tblFrac = [nLowActive nLowInactive; nHighActive nHighInactive];
if all(tblFrac(:) >= 0) && all(sum(tblFrac,2) > 0)
    [~, pooled.stats.fracActive] = fishertest(tblFrac);
else
    pooled.stats.fracActive = NaN;
end

% Build pooled summary table
Tpooled = table( ...
    pooled.low_eventRate_mean, pooled.high_eventRate_mean, ...
    pooled.low_eventRate_median, pooled.high_eventRate_median, ...
    pooled.low_totalEventCount, pooled.high_totalEventCount, ...
    pooled.low_fracActiveNeurons, pooled.high_fracActiveNeurons, ...
    pooled.low_eventAmp_mean, pooled.high_eventAmp_mean, ...
    pooled.low_eventDur_mean, pooled.high_eventDur_mean, ...
    pooled.low_eventAUC_mean, pooled.high_eventAUC_mean, ...
    numel(pooled.low_rates), numel(pooled.high_rates), ...
    numel(pooled.low_amp), numel(pooled.high_amp), ...
    numel(pooled.low_dur), numel(pooled.high_dur), ...
    numel(pooled.low_auc), numel(pooled.high_auc), ...
    pooled.stats.rate, pooled.stats.fracActive, pooled.stats.amp, ...
    pooled.stats.dur, pooled.stats.auc, ...
    'VariableNames', { ...
    'low_eventRate_mean','high_eventRate_mean', ...
    'low_eventRate_median','high_eventRate_median', ...
    'low_totalEventCount','high_totalEventCount', ...
    ...
    'low_eventDur_mean','high_eventDur_mean', ...
    'low_eventAUC_mean','high_eventAUC_mean', ...
    'nNeurons_low','nNeurons_high', ...
    'nEvents_amp_low','nEvents_amp_high', ...
    'nEvents_dur_low','nEvents_dur_high', ...
    'nEvents_auc_low','nEvents_auc_high', ...
    'p_rate_ranksum','p_fracActive_fisher','p_amp_ranksum', ...
    'p_dur_ranksum','p_auc_ranksum'});

disp('================ POOLED NEURON / EVENT ANALYSIS ================')
disp(Tpooled)



%% =========================
% 5C) SUMMARY OF WITHIN-RECORDING NEURON-PAIRED TESTS
% =========================

validDur = isfinite(R.neuronPair_p_dur) & isfinite(R.neuronPair_delta_dur);
validAUC = isfinite(R.neuronPair_p_auc) & isfinite(R.neuronPair_delta_auc);

nSigDur = sum(R.neuronPair_p_dur(validDur) < 0.05);
nPosDur = sum(R.neuronPair_delta_dur(validDur) > 0);
nValidDur = sum(validDur);

nSigAUC = sum(R.neuronPair_p_auc(validAUC) < 0.05);
nPosAUC = sum(R.neuronPair_delta_auc(validAUC) > 0);
nValidAUC = sum(validAUC);

fprintf('\n================ WITHIN-RECORDING NEURON-PAIRED TESTS ================\n');
fprintf('Duration: %d/%d recordings p<0.05, %d/%d with positive median Δ\n', ...
    nSigDur, nValidDur, nPosDur, nValidDur);
fprintf('AUC:      %d/%d recordings p<0.05, %d/%d with positive median Δ\n', ...
    nSigAUC, nValidAUC, nPosAUC, nValidAUC);

if saveOutputs
    writetable(Tpooled, fullfile(outFolder, 'summary_events_active_vs_inactive_pooled.csv'));
    save(fullfile(outFolder, 'summary_events_active_vs_inactive_pooled.mat'), ...
        'pooled', 'Tpooled', '-v7.3');
end
%% =========================
% 6) SUMMARY FIGURE
...
%% =========================
% 6B) POOLED SUMMARY FIGURE
% =========================

if ~isempty(pooled.low_rates) && ~isempty(pooled.high_rates)

    figP = figure('Name','Pooled active vs inactive event comparison', ...
        'Position',[100 100 1400 700], 'Color','w');
    tiledlayout(1,5, 'Padding','compact', 'TileSpacing','compact');

    % Mean/median event rate distribution across neurons
    nexttile; hold on;
    boxchart(ones(numel(pooled.low_rates),1)*1, pooled.low_rates);
    boxchart(ones(numel(pooled.high_rates),1)*2, pooled.high_rates);
    xlim([0.5 2.5]);
    set(gca,'XTick',[1 2],'XTickLabel',{'Least active','Most active'});
    ylabel('Per-neuron event rate (events/s)');
    title(sprintf('Rate\np=%.3g', pooled.stats.rate));

    % Fraction active neurons
    nexttile; hold on;
    bar([1 2], [pooled.low_fracActiveNeurons pooled.high_fracActiveNeurons]);
    xlim([0.5 2.5]);
    set(gca,'XTick',[1 2],'XTickLabel',{'Least active','Most active'});
    ylabel('Fraction active neurons');
    title(sprintf('Frac active\np=%.3g', pooled.stats.fracActive));

    % Amplitude
    nexttile; hold on;
    boxchart(ones(numel(pooled.low_amp),1)*1, pooled.low_amp);
    boxchart(ones(numel(pooled.high_amp),1)*2, pooled.high_amp);
    xlim([0.5 2.5]);
    set(gca,'XTick',[1 2],'XTickLabel',{'Least active','Most active'});
    ylabel('Event amplitude');
    title(sprintf('Amplitude\np=%.3g', pooled.stats.amp));

    % Duration
    nexttile; hold on;
    boxchart(ones(numel(pooled.low_dur),1)*1, pooled.low_dur);
    boxchart(ones(numel(pooled.high_dur),1)*2, pooled.high_dur);
    xlim([0.5 2.5]);
    set(gca,'XTick',[1 2],'XTickLabel',{'Least active','Most active'});
    ylabel('Event duration (s)');
    title(sprintf('Duration\np=%.3g', pooled.stats.dur));

    % AUC
    nexttile; hold on;
    boxchart(ones(numel(pooled.low_auc),1)*1, pooled.low_auc);
    boxchart(ones(numel(pooled.high_auc),1)*2, pooled.high_auc);
    xlim([0.5 2.5]);
    set(gca,'XTick',[1 2],'XTickLabel',{'Least active','Most active'});
    ylabel('Event AUC');
    title(sprintf('AUC\np=%.3g', pooled.stats.auc));

    sgtitle('Pooled neuron / event analysis: tail-defined inactive vs active windows');

    if saveOutputs
        saveas(figP, fullfile(outFolder, 'summary_events_active_vs_inactive_pooled.png'));
    end
end

%% =========================
% 6C) WITHIN-RECORDING NEURON-PAIRED SUMMARY FIGURE
% =========================

figNP = figure('Name','Within-recording neuron-paired tests', ...
    'Position',[100 100 1200 450], 'Color','w');
tiledlayout(1,2, 'Padding','compact', 'TileSpacing','compact');

% Duration
nexttile; hold on;
idx = find(validDur);
y = R.neuronPair_delta_dur(validDur);
p = R.neuronPair_p_dur(validDur);
x = 1:numel(idx);

plot(x, zeros(size(x)), 'k--');
scatter(x(p>=0.05), y(p>=0.05), 50, 'o', 'MarkerEdgeColor','k');
scatter(x(p<0.05),  y(p<0.05),  60, 'filled', 'MarkerFaceColor','k');
xlabel('Recording');
ylabel('Median neuron Δ duration (high - low)');
title('Within-recording paired neuron test: duration');
box off;

% AUC
nexttile; hold on;
idx = find(validAUC);
y = R.neuronPair_delta_auc(validAUC);
p = R.neuronPair_p_auc(validAUC);
x = 1:numel(idx);
...
scatter(x(p<0.05),  y(p<0.05),  60, 'filled', 'MarkerFaceColor','k');
xlabel('Recording');
ylabel('Median neuron Δ AUC (high - low)');
title('Within-recording paired neuron test: AUC');
box off;

%% =========================
% 6D) DELTA LOCOMOTION VS DELTA EVENT DURATION / AUC
% =========================

% Build pooled neuron-level arrays
xLoc_dur = [];
yDur = [];
recIdx_dur = [];
...
for r = 1:nR
    dLoc = allRecDeltaLoc(r);

    if isfinite(dLoc) && ~isempty(allNeuronDeltaDur{r})
        dDur = allNeuronDeltaDur{r}(:);
        keep = isfinite(dDur);
        xLoc_dur = [xLoc_dur; repmat(dLoc, sum(keep), 1)]; %#ok<AGROW>
        yDur     = [yDur; dDur(keep)]; %#ok<AGROW>
        recIdx_dur = [recIdx_dur; repmat(r, sum(keep), 1)]; %#ok<AGROW>
    end

    if isfinite(dLoc) && ~isempty(allNeuronDeltaAUC{r})
        dA = allNeuronDeltaAUC{r}(:);
        keep = isfinite(dA);
        xLoc_auc = [xLoc_auc; repmat(dLoc, sum(keep), 1)]; %#ok<AGROW>
        yAUC     = [yAUC; dA(keep)]; %#ok<AGROW>
        recIdx_auc = [recIdx_auc; repmat(r, sum(keep), 1)]; %#ok<AGROW>
    end
end

% Recording-level summary for proper inference
validRec_delta = isfinite(allRecDeltaLoc) & ...
                 ~cellfun(@isempty, allNeuronDeltaDur);

recDeltaLoc = allRecDeltaLoc(validRec_delta);
recDeltaDurMedian = nan(sum(validRec_delta),1);
recDeltaAUCMedian = nan(sum(validRec_delta),1);

rr = find(validRec_delta);
for k = 1:numel(rr)
    r = rr(k);
    recDeltaDurMedian(k) = median(allNeuronDeltaDur{r}, 'omitnan');
    recDeltaAUCMedian(k) = median(allNeuronDeltaAUC{r}, 'omitnan');
end

% Correlations at recording level
if numel(recDeltaLoc) >= 3
    [rhoDur, pDur] = corr(recDeltaLoc, recDeltaDurMedian, 'type', 'Spearman', 'rows', 'complete');
    [rhoAUC, pAUC] = corr(recDeltaLoc, recDeltaAUCMedian, 'type', 'Spearman', 'rows', 'complete');
else
    rhoDur = NaN; pDur = NaN;
    rhoAUC = NaN; pAUC = NaN;
end

fprintf('\n================ DELTA LOCOMOTION VS DELTA EVENT METRICS ================\n');
fprintf('Recording-level Spearman: Δloc vs median Δduration: rho = %.4g, p = %.4g\n', rhoDur, pDur);
fprintf('Recording-level Spearman: Δloc vs median ΔAUC:      rho = %.4g, p = %.4g\n', rhoAUC, pAUC);

% Figure
figDL = figure('Name','Delta locomotion vs delta event metrics', ...
...
if ~isempty(xLoc_dur)
    scatter_kde_like(xLoc_dur, yDur, 10);
end

% overlay recording medians
scatter(recDeltaLoc, recDeltaDurMedian, 50, 'k', 'filled', 'MarkerEdgeColor','w');

xlabel('\Delta locomotion (bouts/min)');
ylabel('\Delta mean event duration per neuron (s)');
title(sprintf('\\Delta locomotion vs \\Delta duration\nrecording-level Spearman \\rho = %.3f, p = %.3g', rhoDur, pDur));
box off;
...
if ~isempty(xLoc_auc)
    scatter_kde_like(xLoc_auc, yAUC, 10);
end

scatter(recDeltaLoc, recDeltaAUCMedian, 50, 'k', 'filled', 'MarkerEdgeColor','w');

xlabel('\Delta locomotion (bouts/min)');
ylabel('\Delta mean event AUC per neuron');
title(sprintf('\\Delta locomotion vs \\Delta AUC\nrecording-level Spearman \\rho = %.3f, p = %.3g', rhoAUC, pAUC));
box off;

if saveOutputs
    saveas(figDL, fullfile(outFolder, 'delta_locomotion_vs_delta_event_metrics.png'));
end

if saveOutputs
    saveas(figNP, fullfile(outFolder, 'within_recording_neuron_paired_summary.png'));
end
%% ================== HELPERS ==================
...
function [S, durVals, aucVals] = per_neuron_event_features(eveWin, dffWin, fps)
% eveWin, dffWin: time x neurons
    ...
    for n = 1:N
        ...
        nEvents(n) = numel(starts);
        if isempty(starts)
            continue;
        end

        durN = nan(numel(starts),1);
        aucN = nan(numel(starts),1);

        for k = 1:numel(starts)
            idx = starts(k):ends(k);
            seg = x(idx);
            durN(k) = numel(idx) / fps;
            aucN(k) = trapz(seg) / fps;
        end

        meanDur(n) = mean(durN, 'omitnan');
        meanAUC(n) = mean(aucN, 'omitnan');
...
function ex = choose_example_neurons(eveWin, nExample)
% Choose neurons spanning occupancy among active cells

    occ = sum(eveWin,1);
    nz = find(occ > 0);

    if isempty(nz)
        ex = 1:min(nExample, size(eveWin,2));
        return;
    end

    [~, ord] = sort(occ(nz), 'ascend');
    nz = nz(ord);

    idx = round(linspace(1, numel(nz), min(nExample, numel(nz))));
    ex = nz(idx);
end

function plot_example_trace_stack_same_neurons(t, dffWin, eveWin, exNeurons)
% Plot stacked example traces for the SAME neuron IDs across conditions
    ...
    for i = 1:nEx
        ...
        for k = 1:numel(starts)
            plot(t(starts(k)), xPlot(starts(k)) + offset, 'go', 'MarkerSize', 4, 'MarkerFaceColor', 'g');
            plot(t(ends(k)),   xPlot(ends(k))   + offset, 'ro', 'MarkerSize', 4, 'MarkerFaceColor', 'r');
        end

        plot(t, xPlot + offset, 'k-', 'LineWidth', 0.9);
        text(t(1), offset + 0.5, sprintf('cell %d', n), ...
            'HorizontalAlignment','right', 'VerticalAlignment','middle', ...
            'FontSize',8);
    end

    ylim([-0.3 max(offsets)+1.4]);
    set(gca, 'YTick', []);
end

function plot_paired_per_neuron(xLow, xHigh)
    ...
    for i = 1:numel(xLow)
        plot([1 2], [xLow(i) xHigh(i)], '-', 'Color', [0.7 0.7 0.7], 'LineWidth', 0.5);
    end
    scatter(ones(size(xLow)), xLow, 15, 'filled');
    scatter(2*ones(size(xHigh)), xHigh, 15, 'filled');

    medLow = median(xLow, 'omitnan');
    medHigh = median(xHigh, 'omitnan');
    plot([0.85 1.15], [medLow medLow], 'k-', 'LineWidth', 2);
    plot([1.85 2.15], [medHigh medHigh], 'k-', 'LineWidth', 2);
end

function n = count_event_runs(eveWin)
% eveWin: time x neurons
    n = 0;
    for c = 1:size(eveWin,2)
        r = logical(eveWin(:,c));
        d = diff([false; r; false]);
        n = n + sum(d == 1);
    end
...
function fig = make_LL_full_per_fish_QC(eve_fra_cel, dff_fra_cel, low_idx, high_idx, fps, recLabel, nExample)
% Comprehensive per-animal QC panel for LL event detection
% Uses the SAME example neurons in low and high, chosen by strongest change.

    if nargin < 7 || isempty(nExample)
        nExample = 5;
    end
    ...
    % Sort neurons by event occupancy within each window
    occLow  = sum(eveLow, 1, 'omitnan');
    occHigh = sum(eveHigh, 1, 'omitnan');

    [~, ordLow]  = sort(occLow, 'ascend');
    [~, ordHigh] = sort(occHigh, 'ascend');
    ...
    [perNeuronLow, durLow, aucLow]    = per_neuron_event_features(eveLow, dffLow, fps);
    [perNeuronHigh, durHigh, aucHigh] = per_neuron_event_features(eveHigh, dffHigh, fps);

    % SAME example neurons in low and high, chosen by strongest change
    exNeurons = choose_changing_neurons(perNeuronLow, perNeuronHigh, nExample);

    fig = figure('Name', ['Full QC ' recLabel], ...
        'Position', [30 20 1900 1600], ...
        'Color', 'w');
    ...
    % Row 1: rasters
    % =========================
    nexttile(tl, [1 2]); hold on;
    imagesc(tLow, 1:size(eveLowS,1), eveLowS);
    colormap(gca, flipud(gray));
    caxis([0 1]);
    set(gca, 'YDir','normal');
    xlim([tLow(1) tLow(end)]);
    xlabel('Time (s)');
    ylabel('Neurons');
    title(sprintf('Least active raster\nN=%d, total events=%d', ...
        size(eveLowS,1), count_event_runs(eveLow)));
    text(0.01, 0.98, sprintf('Window = %.1f min', numel(low_idx)/fps/60), ...
        'Units','normalized', 'HorizontalAlignment','left', ...
        'VerticalAlignment','top', 'FontSize',10, 'BackgroundColor','w');
    ...
    caxis([0 1]);
    set(gca, 'YDir','normal');
    xlim([tHigh(1) tHigh(end)]);
    xlabel('Time (s)');
    ylabel('Neurons');
    title(sprintf('Most active raster\nN=%d, total events=%d', ...
        size(eveHighS,1), count_event_runs(eveHigh)));
    text(0.01, 0.98, sprintf('Window = %.1f min', numel(high_idx)/fps/60), ...
        'Units','normalized', 'HorizontalAlignment','left', ...
        'VerticalAlignment','top', 'FontSize',10, 'BackgroundColor','w');

    % =========================
    % Row 2: active fraction only
    % =========================
    nexttile(tl, [1 2]); hold on;
    plot(tLow, popLow, 'k-', 'LineWidth', 1.1);
    xlim([tLow(1) tLow(end)]);
    ylabel('Active neuron fraction');
    xlabel('Time (s)');
    title('Least active: active fraction');

    nexttile(tl, [1 2]); hold on;
    plot(tHigh, popHigh, 'k-', 'LineWidth', 1.1);
    xlim([tHigh(1) tHigh(end)]);
    ylabel('Active neuron fraction');
    xlabel('Time (s)');
    title('Most active: active fraction');

    % =========================
    % Row 3: mean dF/F only
    % =========================
    nexttile(tl, [1 2]); hold on;
    plot(tLow, meanDffLow, 'Color', [0.4 0.4 0.4], 'LineWidth', 0.9);
    xlim([tLow(1) tLow(end)]);
    ylabel('Mean dF/F');
    xlabel('Time (s)');
    title('Least active: mean dF/F');

    nexttile(tl, [1 2]); hold on;
    plot(tHigh, meanDffHigh, 'Color', [0.4 0.4 0.4], 'LineWidth', 0.9);
    xlim([tHigh(1) tHigh(end)]);
    ylabel('Mean dF/F');
    xlabel('Time (s)');
    title('Most active: mean dF/F');

    % =========================
    % Row 4: same example traces in low and high
    % =========================
    nexttile(tl, [1 2]); hold on;
    plot_example_trace_stack_same_neurons(tLow, dffLow, eveLow, exNeurons);
    xlim([tLow(1) tLow(end)]);
    xlabel('Time (s)');
    ylabel('Example traces');
    title(sprintf('Least active examples (same neurons, n=%d)', numel(exNeurons)));

    nexttile(tl, [1 2]); hold on;
    plot_example_trace_stack_same_neurons(tHigh, dffHigh, eveHigh, exNeurons);
    xlim([tHigh(1) tHigh(end)]);
    xlabel('Time (s)');
    ylabel('Example traces');
    title(sprintf('Most active examples (same neurons, n=%d)', numel(exNeurons)));

    % =========================
    % Row 5: per-fish summaries
    % =========================
    nexttile; hold on;
    if ~isempty(durLow),  cdfplot(durLow); end
    if ~isempty(durHigh), cdfplot(durHigh); end
    ...
    nexttile; hold on;
    if ~isempty(aucLow),  cdfplot(aucLow); end
    if ~isempty(aucHigh), cdfplot(aucHigh); end
    legend({'Low','High'}, 'Location','southeast');
    xlabel('Event AUC');
    ylabel('ECDF');
    title('AUC ECDF');
    ...
    nexttile; hold on;
    plot_paired_per_neuron(perNeuronLow.meanDur, perNeuronHigh.meanDur);
    xlim([0.5 2.5]);
    set(gca,'XTick',[1 2],'XTickLabel',{'Low','High'});
    ylabel('Mean event duration (s)');
    title('Per-neuron duration');

    nexttile; hold on;
    plot_paired_per_neuron(perNeuronLow.meanAUC, perNeuronHigh.meanAUC);
    xlim([0.5 2.5]);
    set(gca,'XTick',[1 2],'XTickLabel',{'Low','High'});
    ylabel('Mean event AUC');
    title('Per-neuron AUC');

    sgtitle(sprintf('Per-animal LL event QC: %s', recLabel), 'FontWeight', 'bold');
end

function exNeurons = choose_changing_neurons(perNeuronLow, perNeuronHigh, nExample)
% Choose SAME neurons to display in low and high.
% Prioritize neurons that increase duration/AUC from low to high.

    lowDur  = perNeuronLow.meanDur;
    highDur = perNeuronHigh.meanDur;
    lowAUC  = perNeuronLow.meanAUC;
    highAUC = perNeuronHigh.meanAUC;

    dDur = highDur - lowDur;
    dAUC = highAUC - lowAUC;

    valid = isfinite(dDur) & isfinite(dAUC);

    if ~any(valid)
        % fallback: neurons with most events overall
        score = perNeuronLow.nEvents + perNeuronHigh.nEvents;
        [~, ord] = sort(score, 'descend');
        exNeurons = ord(1:min(nExample, numel(ord)));
        return;
    end
    ...
    zDur(valid) = local_zscore(dDur(valid));
    zAUC(valid) = local_zscore(dAUC(valid));

    score = zDur + zAUC;

    % prioritize positive changers
    score(~valid) = -Inf;
...
function out = compute_per_neuron_event_features(eveWin, dffWin, fps)
    nNeurons = size(eveWin, 2);
    out = struct();
    out.meanDur = nan(nNeurons,1);
    out.meanAUC = nan(nNeurons,1);
    out.nEvents = zeros(nNeurons,1);
    ...
    for n = 1:nNeurons
        ...
        for k = 1:numel(starts)
            idx = starts(k):ends(k);
            durN(k) = numel(idx) / fps;
            aucN(k) = trapz(x(idx)) / fps;
        end

        out.meanDur(n) = mean(durN, 'omitnan');
        out.meanAUC(n) = mean(aucN, 'omitnan');
    end
end

function scatter_kde_like(x, y, markerSize)
% Density-colored scatter using local 2D histogram counts
    ...
    for i = 1:numel(x)
        if binX(i) >= 1 && binY(i) >= 1
            dens(i) = N(binX(i), binY(i));
        end
    end

    % plot low density first, high density last
    [~, ord] = sort(dens, 'ascend', 'MissingPlacement','first');
    scatter(x(ord), y(ord), markerSize, dens(ord), 'filled', ...
