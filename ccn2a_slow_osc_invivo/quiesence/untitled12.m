...
% Returns stim.light, stim.tap, stim.lighttap as frame vectors if possible.

    stim = struct('light', [], 'tap', [], 'lighttap', []);

    % -------- Case A: sti_ons_fra is a struct --------
    if isfield(S, 'sti_ons_fra') && isstruct(S.sti_ons_fra)
        F = S.sti_ons_fra;
        stim.light    = get_possible_field(F, {'light','lights','vis','visual'});
        stim.tap      = get_possible_field(F, {'tap','taps','mech','mechanical'});
        stim.lighttap = get_possible_field(F, {'lighttap','light_tap','lights_taps','combined','both'});
        return;
    end

    % -------- Case B: onset vector + type labels --------
    if isfield(S, 'sti_ons_fra') && isnumeric(S.sti_ons_fra)
        ons = S.sti_ons_fra(:);

        typeVars = {'sti_typ','stim_type','sti_type','stimType','event_type','eventType','sti_id','stim_id'};
        typ = [];
        for i = 1:numel(typeVars)
            if isfield(S, typeVars{i})
                typ = S.(typeVars{i});
                break;
            end
        end
        ...
        if ~isempty(typ)
            typ = typ(:);
            n = min(numel(ons), numel(typ));
            ons = ons(1:n);
            typ = typ(1:n);

            if isnumeric(typ)
                % heuristic: 1=light, 2=tap, 3=lighttap
                stim.light    = ons(typ==1);
                stim.tap      = ons(typ==2);
                stim.lighttap = ons(typ==3);
                return;
            elseif iscellstr(typ) || isstring(typ)
                typ = string(typ);
                stim.light    = ons(contains(lower(typ), "light"));
                stim.tap      = ons(contains(lower(typ), "tap"));
                stim.lighttap = ons(contains(lower(typ), "lighttap") | contains(lower(typ), "light_tap") | contains(lower(typ), "both"));
                return;
            end
        end
    end
    ...
    % -------- Case C: Nx2 matrix --------
    if isfield(S, 'sti_ons_fra') && isnumeric(S.sti_ons_fra) && size(S.sti_ons_fra,2) >= 2
        X = S.sti_ons_fra;
        ons = X(:,1);
        typ = X(:,2);
    ...
    % -------- fallback: search for direct variables --------
    vars = fieldnames(S);
    for i = 1:numel(vars)
        nm = lower(vars{i});
        val = S.(vars{i});
        ...
        if max(vv) > nFrames*1.5
            continue;
        end

        if contains(nm, 'light') && isempty(stim.light)
            stim.light = vv;
        elseif contains(nm, 'tap') && ~contains(nm, 'light') && isempty(stim.tap)
            stim.tap = vv;
        elseif (contains(nm, 'lighttap') || contains(nm, 'both') || (contains(nm, 'light') && contains(nm, 'tap'))) ...
                && isempty(stim.lighttap)
            stim.lighttap = vv;
        end
    end
end
