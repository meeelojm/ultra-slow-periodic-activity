...
function [t, y] = load_etho_var(fpath, field)
    S = load(fpath, field, 'fra_tim_uni', 'fra_rat', 'fra_rat_get');
    if ~isfield(S, field)
        error('Field %s not found in %s', field, fpath);
    end
    y = S.(field)(:);
    if isfield(S,'fra_tim_uni') && ~isempty(S.fra_tim_uni)
        t = S.fra_tim_uni(:);
    else
        % rebuild from fps if needed
        fs = [];
        if isfield(S,'fra_rat_get') && ~isempty(S.fra_rat_get), fs = S.fra_rat_get; end
        if isempty(fs) && isfield(S,'fra_rat') && ~isempty(S.fra_rat), fs = S.fra_rat; end
        assert(~isempty(fs), 'Need fra_tim_uni or fra_rat for %s', fpath);
        t = (0:numel(y)-1)'/fs;
    end
    % dedup times
    [t, ia] = unique(t, 'stable'); y = y(ia);
end

function yq = resamp_to(t, y, t_common)
    [tu, ia] = unique(t, 'stable'); yu = y(ia);
    yq = interp1(tu, yu, t_common, 'linear', 'extrap'); % extrap OK: we clipped with intersection
end
